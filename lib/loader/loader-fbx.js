...
var loader_fbx = loader_fbx || {};


loader_fbx.load = function (url, callback) {

    fs.readFile(url, function (err, source) {
        if (err) {
        ...
        if (!loader_fbx.isFbxFormatASCII(text)) {

            console.warn('loader_fbx: !!! FBX Binary format not supported !!!');

        } else if (!loader_fbx.isFbxVersionSupported(text)) {

            console.warn('loader_fbx: !!! FBX Version below 7 not supported !!!');

        } else {

            var res = loader_fbx.parse(text);
...
loader_fbx.isFbxFormatASCII = function (body) {
...
loader_fbx.isFbxVersionSupported = function (body) {
    ...
    if (match) {
        ...
        console.log('loader_fbx: FBX version ' + version);
        return version >= 7000;
...
loader_fbx.parse = function (text) {

    var nodes = new FBXParser().parse(text);

    return this.parseGeometries(nodes);
};

loader_fbx.parseGeometries = function (node) {
...
loader_fbx.parseGeometry = function (node, nodes) {
    ...
    for (var i = 0; i < geo.vertices.length / 3; ++i) {
        vbo.push(geo.vertices[i * 3 + 0]);
        vbo.push(geo.vertices[i * 3 + 1]);
        vbo.push(geo.vertices[i * 3 + 2]);

        vbo.push(geo.normals[i * 3 + 0]);
        vbo.push(geo.normals[i * 3 + 1]);
        vbo.push(geo.normals[i * 3 + 2]);

        vbo.push(geo.uvs[i * 2 + 0]);
        vbo.push(geo.uvs[i * 2 + 1]);
    }

    for (var i = 0; i < geo.indices.length; ++i) {
        ibo.push(geo.indices[i]);
    }

    return {vbo: vbo, ibo: ibo};
};

loader_fbx.addAnimation = function (mesh, matrices, animations) {
...
loader_fbx.parseMaterials = function (node) {
...
// TODO
loader_fbx.parseMaterial = function (node) {
    ...
    parseNodeSpecialProperty: function (line, propName, propValue) {
        ...
        switch (innerPropType1) {
            ...
            case "Vector3D":
                ...
                innerPropValue = [tmp[0], tmp[1], tmp[2]];
                break;
    ...
    for (var b = 0; b < hi.length; ++b) {
        ...
        for (var v = 0; v < clst.indices.length; ++v) {
            ...
            if (affect.length > 1) {
                ...
                console.warn("loader_fbx: node " + clst.id + " have many weight kids: " + affect);
...
Bones.prototype.searchRealId = function (internalId) {
    ...
    // console.warn( 'loader_fbx: notfound internalId in bones: ' + internalId);
    return -1;
...
Bones.prototype.getBoneIdfromInternalId = function (node, id) {
    ...
    // console.warn( 'loader_fbx: bone internalId(' + id + ') not found in bone hierarchy' );
    return -1;
...
Geometry.prototype.parse = function (geoNode) {
    ...
    if (this.vertices === undefined) {
        ...
        console.log('loader_fbx: Geometry.parse(): pass' + this.node.id);
        return;
...
Geometry.prototype.getPolygonVertexIndices = function () {
    ...
    if (topologyN === null) {
        ...
        console.warn("loader_fbx: topology N not found: " + this.node.attrName);
