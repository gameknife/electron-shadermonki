...
loader_osgjs.parseGeometry = Promise.coroutine(function* (node) {
    ...
    if( iboNode !== undefined && vboNode !== undefined )
    {
        ...
        if(stateNode)
        {
            geometry.mainTex = yield loader_osgjs.parseMaterial(stateNode);
        }
...
loader_osgjs.parseMaterial = Promise.coroutine(function* (node) {
    // "AttributeList": [ {
    //     "osg.Material": {
    //         "UniqueID": 10,
    //         "Name": "bench",
    //
    //
    let attribs = node['osg.StateSet']['AttributeList'];
    let material = attribs[0]['osg.Material'];
    let name = material['Name'];
    name += '.mat';
    let matbuffer = yield fs.readFile( path.join(loader_osgjs.currDir, name) );
    return matbuffer.toString();
});
...
loader_osgjs.parseVbo = Promise.coroutine(function* (node, userDataNode, targetIbo ) {
    ...
    for( let i = 0; i < position.length / 3; ++i )
    {
        ...
        vbo.push( position[i * 3 + 0] * 0.2 );
        vbo.push( position[i * 3 + 1] * 0.2 );
        vbo.push( position[i * 3 + 2] * 0.2 );
