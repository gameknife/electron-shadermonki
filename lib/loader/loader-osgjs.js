...
loader_osgjs.parseGeometry = Promise.coroutine(function* (node) {
    ...
    if( iboNode !== undefined && vboNode !== undefined )
    {
        ...
        if(stateNode)
        {
            geometry.mainMaterial = yield loader_osgjs.parseMaterial(stateNode);
        }
...
var prevTex = new glw.createMaterialObject();

loader_osgjs.parseMaterial = Promise.coroutine(function* (node) {

    let attribs = node['osg.StateSet'];
    let material = null;
    ...
    if(material !== null && material['Name'])
    {
        ...
        try {
            ...
            if(buffer)
            {
                ...
                if(matnode)
                {
                    prevTex = glw.createMaterialObject();
                    for( let channel in matnode.channels)
                    {
                        ...
                        if( channelnode.texture )
                        {
                            ...
                            {
                                prevTex.mainTexPath = finalname;
                            }

                            if( channel == 'AOPBR' )
                            {
                                prevTex.lightMapTexPath = finalname;
                            }
                        }
                    }
