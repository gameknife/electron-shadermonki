...
class ShaderMonkiFramework extends Framework
{
    ...
    init() {
        ...
        //this.mainCamera.transform.rotation = gkCore.math.quat.setAxisAngle(gkCore.math.quat.create(), gkCore.math.vec3.fromValues(0,1,0), Math.PI * 1.6 );
        this.mainCamera.transform.rotation = gkCore.math.quat.identity(gkCore.math.quat.create());
    ...
    update() {
        ...
        let sinx = Math.sin(this.camerarotation) * 80.0;
        let cosz = Math.cos(this.camerarotation) * 80.0;

        this.mainCamera.transform.position = gkCore.math.vec3.fromValues(-sinx,0,-cosz);
        this.mainCamera.transform.lookAt( gkCore.math.vec3.fromValues(0,0,0) );
