...
glw.createMeshObject = function ( res, vertexLayout ) {
    ...
    for( let i=0; i < res.length; ++i)
    {
        ...
        let indexStream = res[i].ibo;


        newObj.vbo[i] = glw._create_vbo(vertexStream);
        newObj.ibo[i] = glw._create_ibo(indexStream);

        newObj.indexSize[i] = indexStream.length;
...
class meshObject {
    ...
    draw() {
        ...
        for( let i=0; i < this.indexSize.length; ++i)
        {
            ...
            this.bind();
            this.gl.bindBuffer(this.gl.ELEMENT_ARRAY_BUFFER, this.ibo[i]);
            glw._draw_elements(this.gl.TRIANGLE_STRIP, this.indexSize[i]);
        }
