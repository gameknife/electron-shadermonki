...
glw.createMeshObject = function ( res, vertexLayout ) {
    ...
    for( let i=0; i < res.length; ++i)
    {
        ...
        let vertexStream = res[i].vbo;
        let indexStream_list = res[i].ibo.trilist;
        let indexStream_strip = res[i].ibo.tristrip;

        newObj.vbo[i] = glw._create_vbo(vertexStream);
        if(indexStream_list)
        {
            newObj.trilist[i] = glw._create_ibo(indexStream_list);
            newObj.indexSize_list[i] = indexStream_list.length;
        }
        else
        {
            newObj.indexSize_list[i] = 0;
        }
        if(indexStream_strip)
        {
            newObj.tristrip[i] = glw._create_ibo(indexStream_strip);
            newObj.indexSize_strip[i] = indexStream_strip.length;
        }
        else
        {
            newObj.indexSize_strip[i] = 0;
        }
        newObj.tmpMainMaterial[i] = res[i].mainMaterial;
        if(res[i].worldmatrix)
        {
            newObj.tmpWorldMatrix[i] = res[i].worldmatrix;
        }
        //console.info(res[i].worldmatrix);
    }
    ...
    //console.info('create gl mesh: vert: ' + vertexStream.length +' index: ' + newObj.indexSize);
...
class meshObject {
    ...
    draw( wMat, vpmat ) {
        ...
        for( let i=0; i < this.indexSize_strip.length; ++i)
        {
            if(this.tmpWorldMatrix.length > 0)
            {
                let mMatrix = this.tmpWorldMatrix[i].slice();
                math.mat4.multiply(wMat, mMatrix, mMatrix);
                var mvpMatrix = math.mat4.identity(math.mat4.create());
                math.mat4.multiply(vpmat, mMatrix, mvpMatrix);
                glw.set_uniform4x4fv("_MVP", mvpMatrix);
                glw.set_uniform4x4fv("_M2W", mMatrix);
            }

            if(this.tmpMainMaterial[i] && this.tmpMainMaterial[i].mainTex)
            {
                this.tmpMainMaterial[i].mainTex.load();
                glw.bind_texture( this.tmpMainMaterial[i].mainTex.gltextureobject, 0 );
            }

            if(this.tmpMainMaterial[i] && this.tmpMainMaterial[i].opacityMapTex)
            {
                this.tmpMainMaterial[i].opacityMapTex.load();
                glw.bind_texture( this.tmpMainMaterial[i].opacityMapTex.gltextureobject, 1 );
            }

            this.gl.bindBuffer(this.gl.ARRAY_BUFFER, this.vbo[i]);
            this.bind();

            if(this.indexSize_list[i] > 0)
            {
                this.gl.bindBuffer(this.gl.ELEMENT_ARRAY_BUFFER, this.trilist[i]);
                glw._draw_elements(this.gl.TRIANGLES, this.indexSize_list[i]);
            }
            if(this.indexSize_strip[i] > 0)
            {
                this.gl.bindBuffer(this.gl.ELEMENT_ARRAY_BUFFER, this.tristrip[i]);
                glw._draw_elements(this.gl.TRIANGLE_STRIP, this.indexSize_strip[i]);
            }
...
/// material obj
glw.createMaterialObject = function () {
...
glw._create_framebuffer = function(width, height){
    ...
    return {framebuffer: frameBuffer, depthRenderbuffer: depthRenderBuffer, rendertexture: fTexture};
};
