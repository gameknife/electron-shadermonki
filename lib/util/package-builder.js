...
packageBuilder.build = function() {
    ...
    packageWaitForBuild.forEach( spkgpath => {
        try {
            ...
            packageBuilder.ParsePkg(pkgJSON, pkgDir, pkgResDir);
...
packageBuilder.ParsePkg = function( pkgObject, dir, resdir ){
    ...
    for( let mat in materials )
    {
        ...
        for( let channel in matnode.channels)
        {
            ...
            if( channelnode.texture )
            {
                ...
                packageBuilder.DownloadFile( resdir, toplodurl, name + ext );
