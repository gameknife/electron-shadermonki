...
exporter_obj.normalizeName = function(str) {

    let ret = str.replace(/ /g, "_");
    ret = ret.replace(/#/g, "_");
    return ret;

}

exporter_obj.exportNode = function( rootGameObject, base_path )
{
    ...
    let headerPart = 'mtllib ./exported.mtl\r';
    ...
    mfs.forEach( meshFilter => {
        if( meshFilter.mesh )
        {
            ...
            if( mr )
            {
                facePart += 'usemtl ' + exporter_obj.normalizeName(mr.material.name) + '\r';

                // write mat file here
                if(materialNames.indexOf( exporter_obj.normalizeName(mr.material.name) ) === -1)
                {
                    mtlFileContent += '\r';
                    mtlFileContent += 'newmtl ' + exporter_obj.normalizeName(mr.material.name) + '\r';
                    ...
                    materialNames.push( exporter_obj.normalizeName(mr.material.name) );
                }
    ...
    fs.writeFile(path.join(base_path, 'res', "exported.obj"), headerPart + vertexPart + normalPart + texcoordPart + facePart ,function(err){
        if(!err)
            console.log("obj writed！");
    });

    fs.writeFile(path.join(base_path, 'res', 'exported.mtl'), mtlFileContent ,function(err){
        if(!err)
            console.log("obj writed！");
    });
