...
exporter_obj.exportNode = function( rootGameObject, base_path )
{
    ...
    fs.writeFile(base_path + "exported.obj", headerPart + vertexPart + normalPart + texcoordPart + facePart ,function(err){
        if(!err)
            console.log("obj writed！");
    });

    fs.writeFile(base_path + 'exported.mtl', mtlFileContent ,function(err){
        if(!err)
            console.log("obj writed！");
    });
