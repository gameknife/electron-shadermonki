...
class MeshFilter extends Component {
    ...
    set mesh(value) {
        ...
        for( let i=0; i < vertCount; ++i) {
            ...
            this._lbb = math.vec3.min(this._lbb, this._lbb, position);
            this._rtf = math.vec3.max(this._rtf, this._lbb, position);
        }
        // complete
...
class MeshRenderer extends Component {
    ...
    get bounds() {
        ...
        let lbb_world = math.vec3.transformMat4(math.vec3.create(), mf._lbb, this.host.transform.localToWorldMatrix);

        // transfotm rtf to world
        let rtf_world = math.vec3.transformMat4(math.vec3.create(), mf._rtf, this.host.transform.localToWorldMatrix);

        // return the new bounds
        return {lbb:lbb_world, rtf:rtf_world};
    }
}
